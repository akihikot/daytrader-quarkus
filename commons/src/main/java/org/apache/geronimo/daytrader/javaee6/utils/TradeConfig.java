 ...
 * RunTimeMode (EJB3, DIRECT, SESSION3, JDBC, JPA)
 *
 */

public class TradeConfig {

    public static ObjectMapper JSON_SERIALIZER = new ObjectMapper();
    ...
    // Disable Web JDBC&JPA public static String[] runTimeModeNames = {"Full EJB3",
    // "Direct (JDBC)", "Session (EJB3) To Direct", "Web JDBC", "Web JPA"};
     ...
     * transaction in this period will create a summary and will cache its results.
     */
    private static int marketSummaryInterval = 20;

    /*
     * Penny stocks is a problem where the random price change factor gets a stock
     * down to $.01. In this case trade jumpstarts the price back to $6.00 to keep
     ...
     * decimal precision. At some point, this maximum value can be used to trigger a
     * stock split.
     */
    ...
    public static String webUI[][] = {
            { "/welcome.jsp", "/register.jsp", "/portfolio.jsp", "/quote.jsp", "/tradehome.jsp", "/account.jsp",
                    "/order.jsp", "/config.jsp", "/runStats.jsp" },
            // JSP Interface
            { "/welcomeImg.jsp", "/registerImg.jsp", "/portfolioImg.jsp", "/quoteImg.jsp", "/tradehomeImg.jsp",
                    "/accountImg.jsp", "/orderImg.jsp", "/config.jsp", "/runStats.jsp" },
            // JSP Interface
    };
    ...
    public static char getScenarioAction(boolean newUser) {
         ...
         * portfolio is empty, a buy is executed instead and sellDefecit is incremented.
         * This allows the number of buy/sell operations to stay in sync w/ the given
         * Trade mix.
         */
        ...
        if ((!newUser) && (actions[i] == 'b')) {
            synchronized (TradeConfig.class) {
                if (sellDeficit > 0) {
                    sellDeficit--;
                    return 's';
                    // Special case for TradeScenarioServlet to note this is a buy switched to a
                    // sell to fix sellDeficit
                }
            }
        }
    ...
    public static void setConfigParam(String parm, String value) {
        ...
        if (parm.equalsIgnoreCase("runTimeMode")) {
            try {
                ...
                // >>rjm
                Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "trying to set runtimemode to "
                        + value + "reverting to current value: " + runTimeModeNames[getRunTimeMode()], e);
            ...
            try {
                ...
                Log.error("TradeConfig.setConfigParm(..): minor exception caught"
                        + "trying to set orderProcessingMode to " + value + "reverting to current value: "
            ...
            try {
                ...
                Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "trying to set accessMode to "
                        + value + "reverting to current value: " + accessModeNames[accessMode], e);
            }
        } else if (parm.equalsIgnoreCase("webServicesEndpoint")) {
            try {
                setSoapURL(value);
            } catch (Exception e) {
                Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "Setting web services endpoint", e);
            ...
            try {
                ...
                Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "trying to set workloadMix to "
                        + value + "reverting to current value: " + workloadMixNames[workloadMix], e);
            } // If the value is bad, simply revert to current
            ...
            try {
                ...
                Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "trying to set WebInterface to "
                        + value + "reverting to current value: " + webInterfaceNames[webInterface], e);
            ...
            try {
                ...
                Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "trying to set CachingType to "
                        + value + "reverting to current value: " + cachingTypeNames[cachingType], e);
            } // If the value is bad, simply revert to current
            ...
            try {
                ...
                Log.error("TradeConfig.setConfigParm(..): minor exception caught"
                        + "Setting maxusers, error parsing string to int:" + value + "revering to current value: "
                        + MAX_USERS, e);
            } // On error, revert to saved
            ...
            try {
                ...
                // >>rjm
                Log.error("TradeConfig.setConfigParm(...) minor exception caught"
                        + "Setting max_quotes, error parsing string to int " + value + "reverting to current value: "
                        + MAX_QUOTES, e);
                // <<rjm
            } // On error, revert to saved
            ...
            try {
                ...
                Log.error("TradeConfig.setConfigParm(..): minor exception caught"
                        + "Setting primIterations, error parsing string to int:" + value + "revering to current value: "
                        + primIterations, e);
            } // On error, revert to saved
        }
    }
