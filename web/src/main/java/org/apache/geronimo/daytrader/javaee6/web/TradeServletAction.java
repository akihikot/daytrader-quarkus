...
public class TradeServletAction {
    ...
    public TradeServletAction() {
        if (TradeConfig.getAccessMode() == TradeConfig.STANDARD) {
            ...
            throw new IllegalArgumentException(
                    "TradeServletAction - Uknown TradeConfig accessMode=" + TradeConfig.getAccessMode());
        }
    }
    ...
    void doAccount(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID, String results)
        ...
        try {
            ...
            ArrayList orderDataBeans = (TradeConfig.getLongRun() ? new ArrayList()
                    : (ArrayList) tAction.getOrders(userID));
    ...
    void doLogin(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID, String passwd)
        ...
        try {
            ...
            if (accountData != null) {
                ...
                /*
                 * Invalidate all cookies by, for each cookie received, overwriting value and
                 * instructing browser to delete it; except for the userId cookie; which we will
                 * set to the newly logged in user.
                 */
                ...
                // this is a user error wo will forward them to another page rather than throw a
                // 500
                ...
                Log.debug("TradeServletAction.doLogin(...)" + "Error finding account for user " + userID
        ...
        // If you get here then login failed due to user error so forward them to
        // another page
    ...
    void doLogout(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID)
        ...
        try {
            ...
            /*
             * Invalidate all cookies by, for each cookie received, overwriting value and
             * instructing browser to deletes it
             */
            Cookie[] cookies = req.getCookies();
            if (cookies != null && cookies.length > 0) {
                for (Cookie cookie : cookies) {
